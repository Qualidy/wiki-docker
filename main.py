...
def define_env(env):
    ...
    @env.macro
    def youtube_video(inner_url, title='Video', expanded=False):
        return youtube_video_admonition(inner_url, title, expanded=expanded)
...
def youtube_video_admonition(inner_url, title='Video', expanded=False):
    prefix = '???+ video' if expanded else '??? video'
